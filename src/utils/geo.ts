...
// Approx geohash cell width in km, indexed by precision (string length).
...
/**
 * Returns Firestore range bounds ([start, end) per geohash prefix cell) covering the
 * center point plus its 8 neighboring cells at a precision sized to the radius. Union
 * of these ranges is guaranteed to fully contain the query disk; results still need an
 * exact haversine distance filter afterward since the geohash cells are square-ish.
 */
