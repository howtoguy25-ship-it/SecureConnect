...
import { ALERT_COLORS, ALERT_EMOJI, ALERT_LABELS, type AlertDoc, type AlertType } from "@/types/alert";
...
// which services/alerts.ts's reportAlert then uses as the real Firestore expiresAt for any
// alert this device reports, replacing the app's own per-type default (types/alert.ts's
// ALERT_TTL_MS) for as long as it's set. "Default" (null) reverts to that original per-type
// behavior. Mirrors mobile's SettingsScreen.tsx EXPIRY_PRESETS exactly.
...
export default function App() {
  ...
  const onAlertTypeToggle = useCallback(
    (type: AlertType, value: boolean) => {
      setVisibleAlertTypes((types) => (value ? [...types, type] : types.filter((t) => t !== type)));
    },
    [setVisibleAlertTypes]
  );
  ...
  const onApplyCustomExpiry = useCallback(() => {
    ...
    setAlertExpiryMs(ms);
    setCustomExpiryOpen(false);
  }, [customHoursText, customMinutesText, setAlertExpiryMs]);
      ...
      {!chromeHidden && (
        <div className={`radius-control${topBannerActive ? " chrome-shifted" : ""}${radiusControlExpanded ? "" : " radius-control-collapsed"}`}>
          ...
          {radiusControlExpanded && (
            <>
              {/* Master on/off, matching mobile's Settings screen exactly -- off means no
                  alerts shown/received at all, regardless of radius. Turning it back on resets
                  to a fresh 30km radius, same as mobile's own spec. */}
              ...
              {/* Real, applied override for how long an alert THIS device reports stays live
                  before it auto-expires -- matches mobile's Settings screen exactly. */}
              <div className="radius-helper-text">
