...
Every command below is run from `site-builder/` (this project's root). All assume you're
already logged in (`firebase login`, `eas login`) — both CLIs persist that login on this
machine, so it's a one-time thing per computer, not per session.
...
If you only changed one side and want a faster/narrower deploy:
```
...
**iOS build (EAS) — three profiles already configured in `eas.json`:**
```
